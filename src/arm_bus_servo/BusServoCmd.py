...
serialHandle = serial.Serial("/dev/ttyS2", 115200)  # 初始化串口， 波特率为115200

#GPIO.setwarnings(False)
GPIO.setmode(GPIO.LS2K)

def portInit():  # 配置用到的IO口
    GPIO.setup(RX_EN_PIN, GPIO.OUT)  # 配置RX_CON 即 GPIO17 为输出
    GPIO.output(RX_EN_PIN, GPIO.LOW)
    GPIO.setup(TX_EN_PIN, GPIO.OUT)  # 配置TX_CON 即 GPIO27 为输出
    GPIO.output(TX_EN_PIN, GPIO.HIGH)

portInit()

def portWrite():  # 配置单线串口为输出
    GPIO.output(TX_EN_PIN, GPIO.HIGH)  # 拉高TX_CON 即 GPIO27
    GPIO.output(RX_EN_PIN, GPIO.LOW)  # 拉低RX_CON 即 GPIO17

def portRead():  # 配置单线串口为输入
    GPIO.output(RX_EN_PIN, GPIO.HIGH)  # 拉高RX_CON 即 GPIO17
    GPIO.output(TX_EN_PIN, GPIO.LOW)  # 拉低TX_CON 即 GPIO27

def portRest():
    time.sleep(0.1)
    serialHandle.close()
    GPIO.output(RX_EN_PIN, GPIO.HIGH)
    GPIO.output(TX_EN_PIN, GPIO.HIGH)
    serialHandle.open()
    time.sleep(0.1)
