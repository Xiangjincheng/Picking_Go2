...
def portWrite():  # 配置单线串口为输出
    GPIO.output(TX_PIN, GPIO.HIGH)  # 拉高TX_CON 即 GPIO27
    GPIO.output(RX_PIN, GPIO.LOW)  # 拉低RX_CON 即 GPIO17

def portRead():  # 配置单线串口为输入
    GPIO.output(RX_PIN, GPIO.HIGH)  # 拉高RX_CON 即 GPIO17
    GPIO.output(TX_PIN, GPIO.LOW)  # 拉低TX_CON 即 GPIO27
